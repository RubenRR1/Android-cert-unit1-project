...
class MainActivity : AppCompatActivity() {
    ...
    //Random value for Squeeze text
    private var squeezeText = listOf<String>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        squeezeText = listOf(getString(R.string.squeezeDescription1_en), getString(R.string.squeezeDescription2_en), getString(R.string.squeezeDescription3_en))
    ...
    private fun clickLemonImage() {
        ...
        Log.d(TAG, "Entered ClickLemon fun")
        Log.d(TAG, "Starting state ${lemonadeState}")

        //Check state
        when(lemonadeState) {
            SELECT -> {
                //Generate random size for lemon , call function
                lemonSize = lemonTree.pick()
                //Reset previous squeeze counter value
                squeezeCount = 0
                // Change to next state , squeeze
            ...
            //Check if State is Squeeze
            SQUEEZE -> {
                lemonSize--
                squeezeCount++

                Log.d(TAG, "Presses remaining: ${lemonSize}")
                //If it is, then check if lemon has been squeezed enough times
                if (lemonSize <= 0) {
                    lemonSize = -1
                    lemonadeState = DRINK
                }
        ...
        setViewElements()
        Log.d(TAG, "State changed to ${lemonadeState}")
        Log.d(TAG, "Exit ClickLemon fun")
        Log.d(TAG, "-----------------------------------")
    }
    ...
    private fun setViewElements() {
        ...
        when(lemonadeState)
        {
            SELECT -> textAction.text = getString(R.string.selectDescription_en)
            SQUEEZE ->
            {
                //Removed because it conflicted with LemonadeTest.test_picking_lemon_proceeds_to_squeeze_state
                /*if(squeezeCount==0)
                {
                    textAction.text = getString(R.string.squeezeStart_en)
                }
                else if(lemonSize==1)
                {
                    textAction.text = getString(R.string.squeezeFinal_en)
                }
                else
                {
                    textAction.text = squeezeText.random().toString()//getString(R.string.squeezeDescription1_en)
                }*/
                textAction.text = getString(R.string.squeezeStart_en)
            }
            DRINK -> textAction.text = getString(R.string.drinkDescription1_en)
            RESTART -> textAction.text = getString(R.string.restartDescription_en)
